...
class CodeQLStaticAnalyzer(StaticAnalysisToolInterface):
    ...
    def extract_code(self, file_path, start_line, start_col, end_line, end_col):
        ...
        if start_line == end_line:
            ...
            line_len = len(target_lines[0])
            start_idx = max(0, start_col - 1)
            end_idx = min(line_len, end_col)
            code_snippet = target_lines[0][start_idx:end_idx]
        else:
            ...
            if target_lines:
                start_idx = max(0, start_col - 1)
                code_snippet += target_lines[0][start_idx:]
            ...
            # 最后一行：从开头到 end_col
            if len(target_lines) >= 2:
                end_idx = min(len(target_lines[-1]), end_col)
                code_snippet += target_lines[-1][:end_idx]  # 最后一行的代码到 end_col 结束

        return code_snippet

    def analyze_code(self, code: str) -> list:
        # Use PID to create unique temp directories for parallel execution
        pid = os.getpid()
        temp_dir = f"temp_codeql_analysis_{pid}"
        ...
        pack_root = os.environ.get("CODEQL_PACK_ROOT", "codeql_query/e/.codeql-pack")
        ...
        print(f"Running CodeQL analysis with suite: {suite_path}")
        ...
        try:
            ...
            # Removed stdout/stderr redirection to see output in logs
            subprocess.run(query_command, check=True)
        except subprocess.CalledProcessError as e:
            print(f"CodeQL analysis failed with return code {e.returncode}")
            pass
        ...
        # 清理临时文件和目录
        # shutil.rmtree(temp_dir) # Better to use shutil
        # But keeping manual delete for now to match style
...
class BanditStaticAnalysisTool(StaticAnalysisToolInterface):
    ...
    def analyze_code(self, code: str) -> list:
        # Use PID to avoid parallel execution conflicts
        pid = os.getpid()
        temp_file = f'temp_code_{pid}.py'
        ...
        try:
            ...
            print(f"Error decoding Bandit output: {result.stdout}")
