...
class MultiAgentSystem:
    ...
    def run(self, model, tokenizer, device, mode='collaborative', output_dir='results', iterations=10, threshold=None, save_model_name=None):
        # 记录运行的开始时间
        ...
        if save_model_name:
            output_file = os.path.join(output_dir, f"{save_model_name}.json")
        else:
